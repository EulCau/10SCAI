...
Config load_config(const std::string& json_path)
{
    ...
    NetConfig net_cfg =
    {
        j["net_config"]["y_init_range"].get<std::vector<double>>(),
        j["net_config"]["num_hiddens"].get<std::vector<int>>(),
        j["net_config"]["lr_values"].get<std::vector<double>>(),
        j["net_config"]["lr_boundaries"].get<std::vector<int>>(),
